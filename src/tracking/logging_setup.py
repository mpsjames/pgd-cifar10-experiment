"""Configure per-run and global experiment log files."""
...
class _RunNameFilter(logging.Filter):
...
def configure_run_logger(
    ...
    Side effects:
        - Attaches a `FileHandler` to `log_dir/<run_name>.log`.
    ...
    formatter = logging.Formatter(
        "%(asctime)s [%(levelname)s] %(name)s [%(run_name)s] %(message)s"
    )
    run_filter = _RunNameFilter(run_name)

    per_run = logging.FileHandler(log_dir / f"{run_name}.log", mode="w")
    per_run._pgd_managed = True  # type: ignore[attr-defined]  # sentinel to identify handlers we own
    per_run.setFormatter(formatter)
    per_run.addFilter(run_filter)
